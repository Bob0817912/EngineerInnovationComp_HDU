...
class Car:
    ...
    def __init__(self, com: str, camG: int, camF: int, correct_times: int, center_sz=(238, 246) ,center_wk=(238, 246)):
        """
        :param com: 串口号
        :param cam0: 摄像头0号
        :param cam1: 摄像头1号
        ...
        self.cameraF = Camera.Camera(camF, center_sz=center_sz ,center_wk=center_wk)
        self.cameraG = Camera.Camera(camG, center_sz=center_sz ,center_wk=center_wk, model=True)
    ...
    def move_to_qr(self):
        ...
        车辆行驶到二维码检测区域
        """
        self.uart.car_move_xy_cm(-21, 0, wait=True)
        self.uart.car_move_xy_cm(0, 65, wait=False)
        self.uart.arm_move_angle(angle="out", wait=True)
        self.uart.arm_move_height(height="low", wait=False)

    def qr_task(self):
        """
        扫描二维码，结果保存为task[0],task[1]两个数组
        """
        self.task[0] = [2, 1, 3]
        self.task[1] = [3, 2, 1]
        self.task[0], self.task[1] = self.cameraF.qr_scan()
        print(self.task[0], self.task[1])
        self.uart.send_lcd_tasks(self.task[0], self.task[1], wait=False)
        self.cameraF.close()
    ...
    def material_skip(self, color_skip):
        color, pos = self.cameraG.target_scan("wk")
        if color_skip == color:
    ...
    def material_task(self, task_id: int):
        ...
        for disc_id, color in enumerate(self.task[task_id]):
            while True:
                if color == self.cameraG.target_scan("wk")[0]:
                    if disc_id != 2:
                        self.uart.arm_upload(disc_id, "high")
                    else:  # 若是最后一个物块，在物块放置下去后直接进行下一个动作（无需等待升到最高）
                        self.uart.arm_upload(disc_id, "high", False)
        ...
        while True:
            ...
            if abs(last_pos_x - now_pos_x) >= 10 or abs(last_pos_y - now_pos_y) >= 10:  # 若物块移动，等待至物块静止
                ...
                for index in range(1):  # 对原料区进行校准
                    pos = self.cameraG.target_scan("wk")[1]
    ...
    def place_correct(self, mod: str, color=0):
        ...
        for index in range(self.correct_times):
            if color != 0:
                pos = self.cameraG.target_scan_by_color(color,
                                                        "wk") if mod == "wk" else self.cameraG.target_scan_by_color(
                    color, "sz")
            else:
                pos = self.cameraG.target_scan("wk")[1] if mod == "wk" else self.cameraG.target_scan("sz")[1]
            self.uart.car_move_xy_mm(int((pos[1]) / 5), int((pos[0]) / 5))
    ...
    def place_with_correct(self, mod: str, disc_id: int, height: str, color: int = 0):
        ...
        self.uart.arm_download(disc_id=disc_id, height=height, wait=False)
    ...
    def download_function_first_times_to_place(self, task_id: int, temp_id: int):
        ...
        """
        self.pos_now = 0
        color_now = self.cameraG.target_scan("sz")[0]
        ...
        if color_now == self.task[task_id][0]:  # 若第一个点的颜色为任务的第一个颜色
            ...
            self.color_temp[temp_id][2] = (
                    6 - self.color_temp[temp_id][0] - self.color_temp[temp_id][1]
            )
    ...
    def download_function_second_times_to_place(
            self, task_id: int, temp_id: int, height: str
    ):
        """
        第二次到达某位置放置物料
        :param task_id: 任务号
    ...
    def upload_function(self, task_id: int):
        ...
        for task_index, color_task in enumerate(self.task[task_id]):
            for list_index, color_temp in enumerate(self.color_temp[0]):
                if color_temp == color_task:
...
if __name__ == "__main__":
    # 实例化car对象
    car = Car(com='/dev/ttyUSB0', camG=3, camF=1, correct_times=5, center=(234, 240))
